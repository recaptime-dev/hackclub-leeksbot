...
export const handleMsgAction = async ({
  ...
  if (entry === null) {
    entry = await prisma.slackLeeks.create({
      data: {
        ...
        permalink_message_id: extractPermalink(
          (
  ...
  if (entry.status == "pending") {
    ...
    await sendDM(
      user.id,
      `Thanks for flagging the message privately (ID: \`${entry.message_id}\`) as a leek! If you flagged this for the first time, we'll reach to you via DMs if we approved it. If not, check its status via \`${getBaseSlashCommand()} status ${entry.message_id}\`.`,
    );
    ...
    await sendDM(
      user.id,
      `You flagged as leek to a message (ID: \`${entry.message_id}\`) in this channel using message actions but it is none of our <https://leeksbot.hackclub.lorebooks.wiki/meta/allowlisted-channels|allowlisted channels>. We still logged it on the database just in case.`,
    );
  } else if (entry.status == "rejected") {
    await sendDM(
      user.id,
      `The message you're trying to flag as leek (ID: \`${entry.message_id}\`) via message action was rejected by Leeks Bot Review Queue team (<!subteam^${queueTeam}>). If this is a mistake, please contact one of them via DMs or at <#${metaChannel}> channel`,
    );
  }
};
