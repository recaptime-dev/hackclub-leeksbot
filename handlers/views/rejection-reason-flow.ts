...
export const rejectionReasonHandler = async ({
  ...
  const rejection_reason =
    view.state.values[`${message_id}`]["rejection_reason"].value; // technically bananas
  ...
  try {
    ...
    let originalMessageEmbeds = (
      await slackApp.client.conversations.history({
    ...
    if (entry.status == SlackLeeksStatus.Rejected) {
      ...
      await client.chat.postMessage({
        ...
        blocks: new Blocks([
          new TextSection(
            new MarkdownText(
              `:x: Denied by <@${user.id}> with reason \`${rejection_reason ?? "no reason provided"}\``,
            ),
          ),
        ]).render(),
      });
