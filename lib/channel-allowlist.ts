...
export async function addChannelToAllowlist(channelId: string, admin: string) {
  ...
  if (
    ...
    channelInfo.channel.is_im === true
  ) {
...
export async function removeChannelFromAllowlist(
  channelId: string,
  admin: string,
) {
  // look up the channel first
  ...
  if (data.allowlisted === true || areWeHere === true) {
    ...
    await sendEmpheral(
      admin,
      channelId,
      `You successfully removed <#${channelId}> from the allowlist.`,
    );
  } else if (data.allowlisted === false) {
    await sendEmpheral(
      admin,
      channelId,
      `Channel <#${channelId}> is already blocklisted.`,
    );
  }
}
